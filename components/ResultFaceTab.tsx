...
export interface VFControlProps {
  status: VFStatusUI;
  sessionRemaining: number;     // 남은 생성 횟수
  hasSelection: boolean;
  errorMessage?: string;
  onGenerate: () => void;
}

const GUIDE_LINES: [string, string][] = [["browInnerL", "browInnerR"], ["zygoL", "zygoR"], ["gonionL", "gonionR"], ["foreheadTop", "menton"]];
...
function FacePanel({ data }: { data: FaceResultData }) {
  const [mode, setMode] = useState<"original" | "fitting">("original");
...
function SoftBars({ soft }: { soft: SoftScore[] }) {
  return (<div>
    <div style={{ fontSize: 13, fontWeight: 700, color: C.indigo, marginBottom: 10 }}>얼굴형 (소프트 분류)</div>
...
function ImpressionAxis({ value }: { value: number }) {
  const pos = Math.min(1, Math.max(0, value));
  return (<div>
    <div style={{ fontSize: 13, fontWeight: 700, color: C.indigo, marginBottom: 10 }}>인상결</div>
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.gray, marginBottom: 6 }}><span>소프트 · 커머셜</span><span>샤프 · 비커머셜</span></div>
...
function SymmetryDiag({ items }: { items: DiagItem[] }) {
  return (<div style={{ border: `0.5px solid ${C.line05}`, borderRadius: 8, padding: 12 }}>
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
      <IconAlign /><span style={{ fontSize: 13, fontWeight: 700, color: C.indigo }}>얼굴 대칭·정렬 진단</span>
    </div>
...
function StylingChips({ rec, avoid, selected, onChange }: {
  ...
  return (<div>
    <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
      ...
      <span style={{ fontSize: 11, color: C.gray }}>— 헤어·메이크업을 선택한 뒤 아래 가상 피팅 버튼을 누르세요</span>
    </div>
...
function VFControl({ status, sessionRemaining, hasSelection, errorMessage, onGenerate }: VFControlProps) {
  ...
  else if (!hasSelection) hint = "헤어 또는 메이크업 항목을 선택해주세요";
...
export function ResultTabBar({ active, onChange }: { active: "face" | "body"; onChange: (t: "face" | "body") => void }) {
  const Tab = ({ id, label }: { id: "face" | "body"; label: string }) => {
    const on = active === id;
    return (<button onClick={() => onChange(id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "8px 2px", fontSize: 15, fontWeight: on ? 500 : 400, color: on ? C.black : C.gray, borderBottom: on ? `2px solid ${C.black}` : "2px solid transparent" }}>{label}</button>);
  };
  return (<div style={{ display: "flex", gap: 20, borderBottom: `1px solid ${C.line05}`, marginBottom: 20 }}><Tab id="face" label="얼굴형" /><Tab id="body" label="체형" /></div>);
}
